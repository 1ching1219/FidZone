...
<html>
    ...
    <?php
        ...
        ?>
            ...
            foreach($sql as $s){
                $temp=$s["pId"];
                $rows = "SELECT * FROM posts WHERE pId=$temp AND p_description LIKE '%#娛樂%'";
            ...
            ?>
                <a href="public_details.php?account=<?=$id?>&pId=<?=$temp?>&category=娛樂"><div style="background-image: url('../post_img/<?=$pic[0]?>');" class="personal_post_pic_form"></div></a><br>
            <?php
                }
            ?>
